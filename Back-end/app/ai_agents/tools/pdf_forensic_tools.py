...
def pdf_metadata_extractor(
        ...
        anomalies.append(
            {
                ...
                "description": (
                    ...
                    f"Documentos institucionais devem ser gerados por sistemas de "
            ...
            try:
                if creation_date and not creation_dt:
                    creation_dt = datetime.strptime(
                        creation_date.replace("'", "").replace("Z", "+0000"), fmt
                    )
                    if creation_dt.tzinfo is None:
                        creation_dt = creation_dt.replace(tzinfo=UTC)
                if modification_date and not modification_dt:
                    modification_dt = datetime.strptime(
                        modification_date.replace("'", "").replace("Z", "+0000"), fmt
                    )
                    if modification_dt.tzinfo is None:
                        modification_dt = modification_dt.replace(tzinfo=UTC)
            ...
            anomalies.append(
                {
                    ...
                    "description": (
                        ...
                        f"({modification_date}). Linha temporal impossível — indica "
                        f"adulteração de metadados."
                    ),
                    "confidence": 95,
                }
            )
        ...
        anomalies.append(
            {
                ...
                "description": (
                    ...
                    f"Documentos originais de folha de pagamento não devem ter "
                    f"histórico de edição. "
    ...
    return {
        ...
        "evidence": (
            ...
            f"Incremental saves: {incremental_save_count} | Veredito: "
            f"{('Limpo' if risk_score == 0 else 'Suspeito (' + str(risk_score) + '/100)')}"
        ),
    }
...
def pdf_layer_analyzer(
    ...
    Input:
      ...
      - font_inventory: Semicolon-separated list of font families found
        (e.g. "Helvetica;Arial;TimesNewRoman")
        ...
        anomalies.append(
            {
                ...
                "description": (
                    ...
                    f"Boletos e contracheques institucionais legítimos são "
                    f"documentos de camada única. "
            ...
            anomalies.append(
                {
                    ...
                    "severity": "medium",
                    "description": (
                        f"Detectadas {len(unique_fonts)} famílias de fontes distintas: "
                        f"{unique_fonts}. "
...
def image_forensic_analyzer(
            ...
            anomalies.append(
                {
                    ...
                    "severity": "high",
                    "description": (
                        f"DPI da imagem ({image_dpi}) difere significativamente do DPI do texto "
                        f"({text_dpi}). "
                        f"Isso indica que a imagem foi inserida de fonte externa "
                        f"(ex.: logo copiado da internet) "
    ...
    if image_color_profile and image_color_profile.lower() not in (
        "srgb",
        "devicecmyk",
        "devicergb",
        "",
        ...
        anomalies.append(
            {
                ...
                "description": (
                    ...
                    f"Isso pode indicar que a imagem foi processada em software de "
                    f"edição de imagem "
            ...
            anomalies.append(
                {
                    ...
                    "severity": "low",
                    "description": (
                        f"Imagem posicionada em coordenadas fracionárias "
                        f"({image_position_x:.2f}, {image_position_y:.2f}). "
...
def ocr_confidence_analyzer(
        ...
        anomalies.append(
            {
                ...
                "severity": "high",
                "description": (
                    f"Caracteres isolados com confiança de OCR muito baixa "
                    f"({min_character_confidence:.1f}%). "
                    f"Padrão típico de texto editado manualmente onde a "
                    f"re-renderização degrada a qualidade."
                ),
                "confidence": 80,
            }
        )
            ...
            anomalies.append(
                {
                    ...
                    "severity": "high",
                    "description": (
                        f"{low_confidence_regions}/{total_text_blocks} ({low_ratio:.0%}) "
                        f"regiões de texto "
        ...
        anomalies.append(
            {
                ...
                "severity": "high",
                "description": (
                    "Artefatos de re-amostragem detectados — blurring ou aliasing "
                    "em regiões de texto. "
    ...
    return {
        ...
        "low_confidence_ratio": round(low_confidence_regions / total_text_blocks, 3)
        if total_text_blocks > 0
        else 0,
...
def ai_generation_detector(
    ...
    Input:
      - text_entropy: Shannon entropy of OCR-extracted text
        (higher = more random/less structured)
      - numerical_implausibility_score: How statistically unlikely the
        salary/deduction combo is (0-1)
      - font_rendering_anomaly: Whether characters appear metrically
        inconsistent with known fonts
      - attention_boundary_artifacts: Whether text/background boundaries
        show AI characteristic artifacts
      - inconsistent_baseline: Whether text baseline alignment is inconsistent
      - document_hash_match: Perceptual hash distance from known-authentic
        templates
        ...
        anomalies.append(
            {
                ...
                "description": (
                    ...
                    f"matematicamente válidos mas estatisticamente improváveis para "
                    f"o cargo/região. "
                    f"INCERTEZA: ±25%."
                ),
                "confidence": 65,
