...
_RESPONSE_TEMPLATES = {
    "fraud": (
        "Based on real-time analysis, there are 23 active fraud alerts this month "
        "— a 15% increase from last month. "
        "The top risk areas are: payroll duplication (8 cases), identity mismatch "
        "(7 cases), and overtime anomalies (5 cases). "
        "Critical alerts FR-002 (Ghost Employee — Sales dept, score 91) "
        "and FR-012 (Tax Evasion — Executive, score 93) "
        "require immediate attention. Would you like me to drill into "
        "any specific category?"
    ),
    "risk": (
        "The overall risk score has decreased by 2.3% this month. "
        "The Finance department shows the highest risk concentration "
        "with 12 flagged cases (down from 15 last month). However, "
        "the verification pass rate has improved to 98.4%. "
        "The heatmap shows Operations emerging as a new risk area "
        "(risk score increased from 3.1 to 5.4). "
        "Top 3 risk factors: salary discrepancies (40%), timesheet anomalies "
        "(30%), and document inconsistencies (20%)."
    ),
    "compliance": (
        "Current compliance status: All regulatory checks are passing. "
        "SOC 2 Type II certification is valid through Q4 2026. "
        "There are 3 minor compliance incidents this quarter (all resolved). "
        "No sanctions or blacklist matches in the last 30 days. "
        "LGPD data retention review is due for 7 employee records by Jun 30. "
        "2 vendors flagged for adverse media — "
        "Global Trade Ltda (high risk) and Logistics Express Transportes "
        "(medium risk)."
    ),
    "unusual": (
        "I've detected 2 unusual patterns in the last 24 hours:\n"
        "1. A sudden spike in overtime claims in the Logistics department "
        "— 340% above the weekly average (47h vs 14h). "
        "5 entries were modified outside normal working hours (2-4 AM window).\n"
        "2. 5 payroll adjustments were made outside normal hours "
        "in the Sales department. "
        "Both are flagged for analyst review. Related alerts: FR-005 "
        "(Timesheet Fraud — Operations) and FR-010 "
        "(Timesheet Fraud — Customer Support)."
    ),
    "overtime": (
        "Overtime analysis for the current period:\n"
        "- Logistics: 47h (340% above weekly avg of 14h) — CRITICAL, flagged FR-005\n"
        "- Customer Support: 127h reported in single week (legal max: 56h) "
        "— CRITICAL, flagged FR-010\n"
        "- Engineering: 23h (60% above baseline) — MODERATE, under monitoring\n"
        "- Sales: 18h (within normal range for quarter-end)\n"
        "Recommendation: Audit Logistics and Support timesheets immediately. "
        "Pattern suggests possible timesheet padding or credential sharing."
    ),
    "salary": (
        "Salary anomaly report:\n"
        "- Employee #2841 (John D. Smith, Engineering): Gross pay R$142.5K "
        "— 32% above 95th percentile for role. No promotion memo found.\n"
        "- Employee #5582 (Thomas Moore, Executive): 40% of compensation "
        "routed through offshore entity. Possible tax evasion.\n"
        "- Employee #3120 (Maria Garcia, Sales): Commission structure "
        "appears normal. Salary within band.\n"
        "Top salary discrepancy by department: Engineering (3 cases), "
        "Executive (1 critical case), Sales (1 case)."
    ),
    "department": (
        "Department risk overview:\n"
        "- Engineering: 340 payrolls, risk score 2.1, 3 flagged — LOW risk\n"
        "- Sales: 280 payrolls, risk score 7.8, 14 flagged — HIGH risk "
        "(identity issues, unauthorized deductions)\n"
        "- Operations: 250 payrolls, risk score 5.4, 8 flagged "
        "— MEDIUM risk (overtime anomalies)\n"
        "- Customer Support: 300 payrolls, risk score 8.2, 12 flagged "
        "— HIGH risk (timesheet fraud)\n"
        "- Finance: 120 payrolls, risk score 1.5, 1 flagged — LOW risk\n"
        "- Executive: 50 payrolls, risk score 0.8, 0 flagged — LOW risk "
        "(but 1 critical tax evasion case)"
    ),
    "hello": (
    ...
    "default": (
        ...
        "Would you like me to explore any specific area — fraud alerts, "
        "risk trends, compliance status, or department analysis?"
    ),
}
                    ...
                    messages=[
                        {
                            ...
                            "content": (
                                ...
                                "(CLT, LGPD).\n\n"
                                "CRITICAL: Your DEFAULT BIAS is SKEPTICAL. In fraud detection, "
                                "a FALSE NEGATIVE is CATASTROPHIC. When a user describes "
                                "suspicious patterns, ALWAYS flag them as potential fraud "
                                "and recommend investigation. Never dismiss red flags.\n\n"
